...
**`mobile/` cannot log in at all today.** `mobile/services/AuthService.login`
still POSTs to the deleted `/api/auth/login`; the enrollment screens are TODO-19.
Its token plumbing is finished and correct — `services/tokenStore.ts` holds the
pair and `services/http.ts` attaches `Authorization` and does a single-flight
refresh-and-retry on one 401 — so what is missing is only the way in. `web/`
does have the enrollment flow (`features/auth/EnrollmentPage.tsx`).
