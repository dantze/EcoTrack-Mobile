 ...
 *
 * **This file is a deliberate copy of `web/src/features/sales/idScan/mrz.ts`.**
 * The two projects are built independently, have no shared package and cannot
 * import each other, so the rule is written twice. What stops them drifting is
 * `shared/id-mrz-cases.json`, which both test suites read: change the rule on
 * one side and the other side's suite goes red. Same arrangement, and the same
 * reason, as the fulfilment rule (TODO-41).
 *
 ...
 * line 1's optional-data field, and the name is line 3.
 *
 * **Why this can be trusted to an imperfect recogniser.** Not because the
 * recogniser is good — because the MRZ checks itself. Four ICAO check digits,
 * the CNP's own control digit, and the fact that the CNP restates the birth
 * date and sex that MRZ line 2 states independently. A misread has to survive
 * all of them to get through. So the rule this module implements is:
 *
 *   **autofill ONLY on a fully self-consistent read; otherwise refuse.**
 *
 * That is deliberately strict, and it fails safe: refusing costs the operator
 * two fields of typing, which is what they do today, while a false accept
 * writes a wrong CNP into a client record nobody re-reads.
 *
 * Pure TypeScript on purpose — it imports nothing from react-native, which is
 * what lets `vitest.config.ts` run it with no transform at all. The camera and
 * the recogniser live in `services/IdScanService.ts`.
 */
