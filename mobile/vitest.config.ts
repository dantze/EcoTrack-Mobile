 ...
 * transform chain, and the only logic in this app worth unit-testing imports
 * nothing from react-native: `utils/*` and `types/OrderTypes.ts` are plain
 * TypeScript, so they run with no transform at all. Screens and services are
 * covered by `npm run typecheck` and, for the API layer, by the backend suite.
 *
...
export default defineConfig({
  test: {
    environment: 'node',
    include: ['{utils,types,constants}/**/*.test.ts'],
